...
class ContextBroker:
    ...
    def __init__(self, project_id: str):
        ...
        # IMP-124: which path generated the candidate set on the last retrieve()
        # ("lancedb-ann" when the VDB produced candidates, else "json-fullscan").
        self.last_candidate_source = "json-fullscan"
    ...
    def retrieve(
        ...
        # IMP-124: when LanceDB is active, the candidate set is generated by ANN
        # (vector) + FTS instead of an unconditional JSON full-scan, so the VDB
        # contributes recall and scale rather than only re-ranking. The JSON store
        # stays the SSoT for verbatim content/metadata and the fallback: if LanceDB
        # is absent, or returns nothing (empty table / mid-query degradation), or its
        # candidates don't resolve to known chunks (stale rows), we fall back to the
        # full scan so no SSoT chunk becomes unreachable.
        all_chunks = self.data.get("chunks", [])
        chunks_by_id = {chunk["chunk_id"]: chunk for chunk in all_chunks}
        candidate_chunks = all_chunks
        self.last_candidate_source = "json-fullscan"
        if self._table is not None and lancedb_candidates:
            ann_chunks = [chunks_by_id[cid] for cid in sorted(lancedb_candidates) if cid in chunks_by_id]
            if ann_chunks:
                candidate_chunks = ann_chunks
                self.last_candidate_source = "lancedb-ann"
        scored = []
