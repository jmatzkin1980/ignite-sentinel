...
`/backlog` also emits `04_backlog/SLICE-PLAN.md` plus `08_context_packs/slice_plan.json`. These artifacts order the handoff for downstream planning agents using existing story dependencies, EPIC-002 enabler links, DoR state, readiness score, execution contracts, retrieval plans and anchors. They deliberately stop at the seam: Ignite exposes ordering and context, but does not create task IDs or execute implementation/testing work.
...
The eval harness covers discovery, brief, PRD, specs, and backlog. Backlog answer keys check expected stories/source units, no-invention behavior, slicing-pattern baseline, opt-in anchor/context metrics, and story-quality scoring for the governed INVEST/SPIDR/Lawrence model. Retrieval evals run through the unit suite (`tests/test_evals_retrieval.py`) and write gitignored JSON reports under `tests/evals/reports/`, including metrics by active backend (`json-hybrid` or `lancedb-hybrid`) and golden queries across all eval fixtures.
