...
- **`/refine-backlog`** — agents propose reslicing, split/merge, missing stories, or enabler candidates; accepted proposals land under `04_backlog/refinements/` as `Origin: agent` proposals only. Enabler candidates must state the concrete enabled capability and measurable verification method.
