...
`/validate` keeps structural validity separate from maturity: it returns non-zero only for structural problems, while `requirement_quality`, `semantic_quality`, and `cross_artifact_consistency` emit non-blocking warnings (vague requirement prose with category/why-it-matters detail, scaffolding content, missing EARS/spec-unit continuity, dangling pointers, PRD/spec drift) as corrective guidance.
