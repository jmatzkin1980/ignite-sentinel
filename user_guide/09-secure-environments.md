...
- `/doctor` reports `memory dependency: lancedb (optional)` and `memory backend mode` as local-restricted `WARN`, explicitly marking this as a healthy no-install state, and the verdict stays `PASS`.
