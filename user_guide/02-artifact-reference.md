...
- `valid_at` and `invalid_at` (IMP-152): the fact's bitemporal validity window. `valid_at` records when the system recorded the fact as valid (this materialization); `invalid_at` is `None` while the fact is current. Every materialized unit is current here because the ledger is a projection rebuilt from the current source-of-truth tables; superseded facts keep their row with `invalid_at` set instead of being dropped (IMP-153). The payload also records `materialized_at`.
