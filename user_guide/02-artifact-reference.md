...
`/ingest` (discovery) and `/sync` also emit **focus packs** — `discovery_focus.json` and `sync_focus.json` (IMP-127) — automatically. These carry pointer-only entries (a `summary` plus a `read_plan` anchor, never the verbatim body) so review consults focus instead of re-reading whole artifacts. Multi-section packs (`specs_generation.json`, `backlog_generation.json`) additionally report a `disclosure_budget` block (`used_chars`, `deduped_chunks`, `truncated_chunks`): the pack honors a global character ceiling and de-duplicates chunks across sections while keeping at least one result per section. Packs whose candidates were generated by LanceDB also record `candidate_source` (`lancedb-ann` or `json-fullscan`, IMP-124).
