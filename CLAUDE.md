...
python -m sentinel /specs | /backlog | /refine-backlog --source FILE | /quality | /trace | /health | /validate
...
Gates implementados (no forzarlos): `/specs` y `/backlog` requieren ingest previo y fallan con maturity `BLOCKED`; `/backlog`, `/refine-backlog` y `/quality` se bloquean con health `DIRTY`; `/quality` requiere user stories existentes. `/refine-backlog` solo acepta propuestas agénticas citadas y las registra como overlay `origin: agent`; no reescribe historias ni el boundary de enablers.
