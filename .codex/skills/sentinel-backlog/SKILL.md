...
4. Review `workspaces/PROJECT_ID/04_backlog/EPIC-001.md` as the primary backlog artifact. It contains the epic, story map, slicing rationale, retrieved context summary, and embedded stories.
5. Review generated `US-00x.md` files only as story-level traceability and quality handoff mirrors.
6. Run `python -m sentinel /quality PROJECT_ID`.
7. Run `python -m sentinel /trace PROJECT_ID` and `python -m sentinel /health PROJECT_ID`.
...
- Acceptance criteria must be declarative Given/When/Then scenarios covering happy path, validation path, failure/recovery path, and quality evidence path.
