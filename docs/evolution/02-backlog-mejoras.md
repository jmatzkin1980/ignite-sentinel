...
- Estado: VERIFIED & PUSHED (2026-06-15, branch `imp-066-scrutinize`, PR #64: https://github.com/jmatzkin1980/ignite-sentinel/pull/64; `/scrutinize` + skill/adapters/MCP/doctor/docs/tests/evals; `powershell -ExecutionPolicy Bypass -File .\verify.ps1` verde: 173 tests OK, `/doctor` PASS con warnings opcionales esperados, discovery evals OK con `scrutiny_ok=True`). Pendiente: merge manual del usuario.
...
- Estado: VERIFIED & PUSHED (2026-06-15, branch `imp-067-assume`, PR #65: https://github.com/jmatzkin1980/ignite-sentinel/pull/65; `/assume` + registro `assumptions.md`, ledger `ASSUMED`, maturity/status risk summary, brief/PRD citations, skill/adapters/MCP/doctor/docs/tests/evals; `powershell -ExecutionPolicy Bypass -File .\verify.ps1` verde: 180 tests OK, `/doctor` PASS con warnings opcionales esperados, discovery evals OK con `assumption_ok=True`). Pendiente: merge manual del usuario.
...
- Estado: DONE (2026-06-19, branch `codex/imp-068-readiness-matrix`, PR #66 mergeado por el usuario: https://github.com/jmatzkin1980/ignite-sentinel/pull/66; `development_readiness.json` con matriz de 16 áreas×lente, estados `CONFIRMED`/`ASSUMED`/`OPEN`, evidencia, scores por lente/global, Crystallization Gate; expuesto en `/maturity`, `/status` y dashboard; docs/tests/evals actualizados; `python -m sentinel.adapters` sin drift; `powershell -ExecutionPolicy Bypass -File .\verify.ps1` verde: 182 tests OK, `/doctor` PASS con warnings opcionales esperados, discovery evals OK con `development_readiness_ok=True`).
...
| Fecha | Cambio |
|---|---|
| 2026-06-24 | IMP-119 IMPLEMENTED en `codex/imp-119-implementability-probe` (Horizonte 14/MIR, doc `19`, **cierra el horizonte**): sonda de implementabilidad agéntica pre-flight como **sub-modo de `/scrutinize`** (`--mode implementability-probe`), no comando nuevo — reusa la maquinaria de scrutiny y no agrega superficie de comando (espejo de la decisión de IMP-112). Un agente declara, por `RU-*`, qué le falta para implementar; cada finding exige `unit` (`RU-NNN`, validado contra `requirement_units.md` cuando existe) + cita verbatim local, `finding_type` ∈ {`missing-context`, `non-inferable-gap`, `ambiguous-for-implementation`}, y mergea como `origin: implementability-probe`. Espejo pre-flight de `/implementation-feedback`: señala sin auto-resolver. Reporte por RU `01_discovery/implementability_probe_report.md`, nodo de traza `implementability_probe` (`probed_by`), `gap_counts.implementability_probe_origin`, schema `implementability_probe.schema.json`. Default `--mode scrutiny` intacto (retrocompatible). `tests/test_implementability_probe.py` (9 tests), manifest body + 5 superficies + skill discovery sincronizados (`out_of_sync()==[]`). `verify.ps1` verde. |
| 2026-06-24 | IMP-118 IMPLEMENTED en `codex/imp-118-implementability-matrix` (Horizonte 14/MIR, doc `19`): madurez = implementabilidad por agente. `development_readiness.json` suma `unit_implementability` — matriz por Requirement Unit × lente derivada de los gaps anclados a cada RU (columna `Unit` de IMP-116). Nuevo status `DEFERRED_TO_CONTEXT`: una celda es `OPEN` solo si el faltante es no-inferible (el check lee el texto del cliente, `evidence_scope: source` → solo el cliente puede responder); los scopes de dominio (technical/design/quality/frontend) son descubribles vía context pack y no cuentan como `OPEN` ni frenan lo chico. `/maturity` y `/status` exponen qué RUs son `implementable` / `deferred_to_context` / `not_implementable`; `maturity_score` previo intacto (dimensión aditiva). Gate gobernante soft (espejo de `brief_gate`): por default advierte las RUs no implementables; `implementability_gate.strict` opt-in frena `READY_FOR_SPECS` en `/brief`. `tests/test_unit_implementability.py` (13 tests, pure + integración + gate). No toca la matriz de rubric ni `crystallization_gate` (anti-degradación); sin RUs es no-op (retrocompatible). `verify.ps1` verde (308 tests, /doctor PASS, evals sin regresión). |
| 2026-06-24 | IMP-117 IMPLEMENTED en `codex/imp-117-counterpart-anchoring` (Horizonte 14/MIR, doc `19`): fin de la falsa madurez para conceptos-superficie. Nueva regla declarativa `mention_requires_counterpart` (dispara solo cuando el concepto está nombrado y falta su contracara; si no se menciona, no pregunta) más dos checks `medium`: `GAP-METRIC-DEFINITION` (business; métrica/kpi/indicador sin definición/fuente/umbral) y `GAP-AUTH-MODEL` (technical; auth/login/permiso/rol sin método/modelo/catálogo). No toca `mention_without_counterpart` ni `has_positive_token_evidence` (anti-degradación). Mappings human-facing ES/EN completos, `tests/test_counterpart_anchoring.py`, `GAP-METRIC-DEFINITION` promovido a `must_fire` en ops-risk-backlog, docs/adapters sincronizados. Evals sin regresión (ids nuevos no están en `must_not_fire`). |
| 2026-06-24 | IMP-116 IMPLEMENTED en `codex/imp-116-per-unit-lenses` (Horizonte 14/MIR, doc `19`): los lentes corren por Requirement Unit — `detect_unit_anchored_gaps` ancla cada gap al `RU-*` cuya evidencia citada lo origina y escopa la evidencia por unidad, de modo que un token presente en una RU ya no suprime un gap inquisitivo de otra. Columna `Unit` aditiva en la tabla de trazabilidad de `gaps.md` (retrocompatible, `N/A` en gaps de documento y workspaces previos), campo `unit` opcional en `gap.schema.json` y en `validate_agent_gaps`, `tests/test_gap_unit_anchoring.py`, docs y adapters sincronizados. Sin RUs el comportamiento es idéntico a `detect_gaps`. |
| 2026-06-24 | Reconciliación de estado (verificada contra `git log` en `main`): IMP-110/111/112 (PR #109, `cf58690`), IMP-113 (PR #110, `5ed2f62`), IMP-114 (PR #111, `0b9757c`) e IMP-115 (PR #112, `cfe10a3`) ya estaban MERGED pese a figurar como PENDING en `18-*`/`19-*`. El máximo IMP mergeado pasa a ser IMP-116; frente abierto real: IMP-117…127. |
